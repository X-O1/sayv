...
contract SayvVault {
    ...
    constructor(address _token, uint256 _numOfTokenDecimals, address _addressProvider) {
    ...
    function depositToVault(uint256 _amount, bool _repay) public {
        ...
        emit Deposit_To_Vault(msg.sender, address(i_vaultToken), _amount); // Log deposit
    }
    ...
    function _withdrawFromPool(uint256 _amount, address _to) private {
        i_activePool.withdraw(address(i_vaultToken), _amount, _to);
        emit Withdraw_From_Pool(address(i_vaultToken), _amount, _to); // Logs withdraw
    }
